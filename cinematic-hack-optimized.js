...
setTimeout(createHiddenTrap, 5000);
    ...
    // Блокируем закрытие вкладки/окна - ОБЪЕДИНЕННАЯ ВЕРСИЯ
    beforeUnloadHandler = function(e) {
        if (navigationBlocked) {
            e.preventDefault();
            e.returnValue = 'СИСТЕМА ЗАРАЖЕНА! Закрытие вкладки может повредить компьютер!';
            return 'СИСТЕМА ЗАРАЖЕНА! Закрытие вкладки может повредить компьютер!';
        }
    };
...
// Глобальные функции для управления окнами - с ошибками!
...
function showWindowError(event, title, message) {
    ...
    error.style.cssText = `
        ...
        font-size: 11px;
        z-index: 1000000;
        animation: error-window-appear 0.3s ease-out forwards;
        box-shadow: 0 0 20px rgba(255, 0, 0, 0.8);
    `;
    
    error.innerHTML = `
        <div style="background: #cc0000; padding: 4px 8px; font-weight: bold; display: flex; justify-content: space-between; align-items: center;">
            <span>⚠ ${title}</span>
            <span style="cursor: pointer;" onclick="this.parentElement.parentElement.remove()">×</span>
        </div>
        <div style="padding: 8px;">
            <div style="margin-bottom: 8px;">${message}</div>
            <div style="font-size: 10px; color: #ffcccc;">
                Код ошибки: 0x${Math.floor(Math.random() * 0xFFFF).toString(16).toUpperCase()}
            </div>
            <div style="font-size: 10px; color: #ffcccc;">
                Время: ${new Date().toLocaleTimeString()}
            </div>
            <div style="margin-top: 8px; text-align: center;">
                <button onclick="this.closest('.window-error').remove()" style="
                    background: #cc0000; 
                    color: #fff; 
                    border: 1px solid #fff; 
                    padding: 2px 8px; 
    ...
    setTimeout(() => {
        if (error.parentNode) {
            error.style.animation = 'error-window-disappear 0.3s ease-out forwards';
            setTimeout(() => {
                if (error.parentNode) {
                    error.remove();
                }
    ...
    let moveCount = 0;
    const maxMoves = 8; // Максимум движений
    
    const moveWindow = () => {
        if (!window.parentNode || moveCount >= maxMoves) return;
        
        moveCount++;
        
        // Случайное движение с увеличивающейся интенсивностью
        const intensity = moveCount * 20;
        const randomX = (Math.random() - 0.5) * intensity;
        const randomY = (Math.random() - 0.5) * intensity;
        
        window.style.transition = 'transform 1.5s ease-in-out';
        window.style.transform = `translate(${randomX}px, ${randomY}px)`;
        
        // Добавляем эффект "тряски" для драматизма
        if (moveCount > 3) {
            window.style.animation = 'window-shake 0.5s ease-in-out';
            setTimeout(() => {
        ...
        // Консольные сообщения от "злоумышленника"
        const hackerMessages = [
            'Система адаптируется...',
            'Перехватываю управление окнами...',
            'Блокирую пользовательский интерфейс...',
            'Устанавливаю контроль над дисплеем...',
            'Финальная фаза захвата...',
            'Полный контроль установлен',
            'Сопротивление бесполезно',
            'Добро пожаловать в мою систему'
        ];
        
        if (moveCount <= hackerMessages.length) {
            console.log(`%c[ЗЛОУМЫШЛЕННИК] ${hackerMessages[moveCount - 1]}`, 'color:#00ff00;font-size:12px;font-style:italic');
        }
        
        // Планируем следующее движение
        const nextMoveDelay = Math.random() * 3000 + 2000; // от 2 до 5 секунд
        setTimeout(moveWindow, nextMoveDelay);
    };
    
    // Начинаем движение через случайную задержку
    const initialDelay = Math.random() * 2000 + 1000; // от 1 до 3 секунд
    setTimeout(moveWindow, initialDelay);
}
    ...
    terminal.innerHTML = `
        ...
        ">
            <div style="display: flex; align-items: center; gap: 8px;">
                ...
                <span>СИСТЕМА ВОССТАНОВЛЕНИЯ - КРИТИЧЕСКАЯ ОШИБКА</span>
            </div>
        ...
        <div class="terminal-content" style="
            ...
            line-height: 1.4;
        ">
            <div style="color: #ff0000; font-weight: bold; margin-bottom: 10px;">ВНИМАНИЕ! ОБНАРУЖЕНО ВТОРЖЕНИЕ В СИСТЕМУ!</div>
            <div style="margin-bottom: 8px;"></div>
            <div style="color: #ffff00;">Анализ системы:</div>
            <div style="margin-left: 20px; margin-bottom: 8px;">
                <div>- Источник атаки: ${userInfo.ip}</div>
                <div>- Местоположение: ${userInfo.location}</div>
                <div>- Система: ${userInfo.osName} ${userInfo.deviceType}</div>
                <div>- Браузер: ${userInfo.browserName}</div>
                <div>- Время атаки: ${new Date().toLocaleString()}</div>
            </div>
            <div style="color: #ff0000; font-weight: bold; margin-bottom: 8px;">СТАТУС: СИСТЕМА ПОЛНОСТЬЮ СКОМПРОМЕТИРОВАНА</div>
            <div style="margin-bottom: 8px;"></div>
            <div style="color: #ffff00;">Обнаружены следующие угрозы:</div>
            <div style="margin-left: 20px; margin-bottom: 10px;">
                <div>• Бэкдор установлен и активен</div>
                <div>• Кейлоггер записывает нажатия клавиш</div>
                <div>• Личные данные переданы злоумышленникам</div>
                <div>• Система мониторинга активна</div>
            </div>
            <div style="color: #ffff00; font-weight: bold; margin-bottom: 5px;">ДЛЯ ВОССТАНОВЛЕНИЯ КОНТРОЛЯ НАД СИСТЕМОЙ</div>
            <div style="color: #ffff00; margin-bottom: 10px;">ТРЕБУЕТСЯ НЕМЕДЛЕННАЯ АУТЕНТИФИКАЦИЯ</div>
            <div style="margin-bottom: 8px;">Введите команду для аутентификации:</div>
            <div style="color: #00ff00; margin-bottom: 8px; font-weight: bold;">Формат: auth ваш_ответ</div>
            <div style="color: #ffff00; margin-bottom: 10px;">Пример: auth фурманов семён михайлович</div>
            <div style="color: #ff0000; font-weight: bold; margin-bottom: 10px;">ВОПРОС: <span id="question-text">Полное имя системного администратора</span></div>
        ...
        <div class="terminal-input-area" style="
            ...
            gap: 8px;
        ">
            <span style="color: #ff0000; font-weight: bold;">recovery@system:~#</span>
...
function setupTerminalInput(terminal) {
    ...
    // Начальное сообщение
    addToOutput(output, 'Система готова к аутентификации...', '#ffff00');
    addToOutput(output, 'Введите "help" для списка команд', '#00ff00');
    addToOutput(output, '', '#00ff00');
    addToOutput(output, 'БЫСТРЫЙ СТАРТ:', '#ffff00');
...
function processCommand(command, output, terminal) {
    ...
    addToOutput(output, `recovery@system:~# ${command}`, '#ff0000');
...
const questions = [
    { question: 'Полное имя системного администратора', answer: 'фурманов семён михайлович' },
    { question: 'Метод кодирования данных в целевой системе', answer: 'в двоичном коде' },
    { question: 'Размер единицы выделения памяти в килобайтах', answer: '1024' }
];
